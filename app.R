...
ui <- fluidPage(
  ...
  div(class = "description-text",
      ...
      br(),
      "本ゲームのアルゴリズムの考え方と根拠については，",tags$a(href="https://sites.google.com/view/takahashistat/profile/jbsquesttribute", "JBSクエスト・トリビュートのアルゴリズム", target="_blank", rel="noopener noreferrer"),"で説明しています．", br(),
  ...
  div(class = "footer-text",
      ...
      "独自性について：当時のソースコードは非公開のため，本ゲームはJBSクエストを完全再現したものではなく，製作者がR言語を用いて独自に構築した",tags$a(href="https://sites.google.com/view/takahashistat/profile/jbsquesttribute", "アルゴリズム", target="_blank", rel="noopener noreferrer"),"に基づいています．", br(),
...
server <- function(input, output, session) {
  ...
  execute_turn <- function() {
    ...
    if (c1$hp > 0 && c2$hp > 0) {
      ...
      for (i in 1:2) {
        ...
        # --- 2回連続攻撃の判定 ---
        prob_double <- max(0, (atk_unit$spd - def_unit$spd) / 100) * 0.375
  ...
  # --- SNSシェアボタンの生成 ---
  create_share_button <- function(target_mode) {
    if (!battle_state$finished || battle_state$mode != target_mode) return(NULL)
  ...
  output$share_ui_m1 <- renderUI({ create_share_button(1) })
  output$share_ui_m2 <- renderUI({ create_share_button(2) })
}
