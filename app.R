...
ui <- fluidPage(
  ...
  div(class = "description-text",
      "任意のヒットポイント（HP），攻撃力（ATK），守備力（DEF），素早さ（SPD）のキャラクターを生成してバトルさせるゲームです．それぞれのステータスの最小値は0，最大値は100ですが，4つのステータスの合計値は100以下でなければなりません．", br(),
      ...
      br(),
      "注意事項：", "JBSクエストのソースコードはマル秘なので，本ゲームはJBSクエストを完全再現したものではありません．本ゲームは，製作者がR言語で独自に作成したものです．また，本ゲームはファンによる非公式の二次創作物（トリビュート作品）であり，株式会社集英社および当時の関係者とは一切関係ありません．", br(),
